...
// These are all inline function definitions... Yes, we are lazy, we don't like to have two files ;)
// Besides, it works ;)
...
TString getSignalHistoName(double primaryParameter, int likelihoodType, double other){
    ...
    // The primary parameter is the one that gets plot on X axis of limit ;) by default
...
TString getSignalFile(double primaryParameter, int likelihoodType, double other){
    ...
    // "other" parameter is not used, but are there for your convenience (in case you want to modify)
